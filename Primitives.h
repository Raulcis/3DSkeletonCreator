...
public:
	Joint(glm::vec3 p, float r, ofColor diffuse, int count) { position = p; radius = r; diffuseColor = diffuse;}

	Joint(glm::vec3 p, float r, ofColor diffuse) { position = p; radius = r; diffuseColor = diffuse; }
	Joint() {}
	bool intersect(const Ray& ray, glm::vec3& point, glm::vec3& normal);
	void draw();
	void setname(int count) { name = "Joint" + count++; }
