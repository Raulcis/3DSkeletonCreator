...
void Joint::draw() {
	...
	//   matrix. now all vertices dran will be transformed by this matrix
	/*
	toParent =  normalized vector pointing from the joint to its parent
	defaultConeRot = <0, 1, 0> // Just how OF default points cones
	rot = rotateToVector(defaultConeRot, toParent)
	trans = glm::translate of joint's position
	ofMultMatrix(trans * rot)
	draw cone with length of toParent 
	*/
	ofPushMatrix();	
	if (parent) {
		aimParent = glm::normalize(this->getPosition() - parent->getPosition());

		coneRot = glm::normalize(glm::vec3(0, 1, 0));

		rot = rotateToVector(coneRot, aimParent);
		aimToJoint = parent->getPosition() - ((glm::distance(parent->getPosition(), this->getPosition())) / 2) * glm::normalize(parent->getPosition() - this->getPosition());
		trans = glm::translate(glm::mat4(1.0), aimToJoint);

		ofMultMatrix(trans * rot);
		// distance between the two spheres - diameter of the spheres 
		coneHeight = glm::distance(parent->getPosition(), this->getPosition()) - 2 * Sphere::radius;
		ofDrawCone(radius, coneHeight);
		ofPopMatrix();
